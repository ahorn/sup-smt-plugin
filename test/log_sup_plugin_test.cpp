...
using namespace sup;
typedef LogSupPlugin::Nats Nats;

TEST(LogSupPluginTest, Init)
{
  LogSupPlugin sup_plugin(2);

  EXPECT_TRUE(sup_plugin.group_by(0).empty());
  EXPECT_TRUE(sup_plugin.group_by(1).empty());
}

TEST(LogSupPluginTest, AppendNat)
{
  LogSupPlugin sup_plugin(2);
  ...
  it = sup_plugin.group_by(0).begin();
  EXPECT_EQ(0, *it++);
  EXPECT_EQ(0, *it++);
  EXPECT_EQ(7, *it);
  ...
  it = sup_plugin.group_by(1).begin();
  EXPECT_EQ(0, *it++);
  EXPECT_EQ(8, *it);
}

TEST(LogSupPluginTest, Sup)
{
  LogSupPlugin sup_plugin(8);
...
TEST(LogSupPluginTest, Update)
{
  LogSupPlugin sup_plugin(1);
